 ...
 *
 * Version 2.3 (Test-Fixed):
 * - Wrapped Electron-specific code in an `if (app)` block to allow
 * the file to be imported in a standard Node.js test environment.
 */
...
function readSettings(customPath = null) {
    ...
    // Use custom path for testing, otherwise use the app's default user data path
    // In a test environment, `app` will be undefined, so we must rely on `customPath`.
    const userDataPath = customPath || (app ? app.getPath('userData') : '');
    if (!userDataPath) {
         // This case will happen during tests, which is fine as the test provides the path.
         // If it happens in production, it's an issue with Electron.
         console.log("User data path is not available. This is normal during tests.");
    }
...
// --- The following code will only run in the Electron environment ---
if (app) {
    ...
    async function checkTarget(target, timeout, activeProfileId) {
        ...
        if (previousState.status && previousState.status !== result.status) {
            result.lastChange = new Date().toISOString();
...
// Add this line at the very end for testing purposes
